...
<script type="text/javascript">
    ...
    function deleteBatch() {
        ...
        if (zhi.length == 0) {
            ...
            // 将提交的商品的id值拼接成一个字符串
            if (confirm("您确定删除这" + zhi.length + "条商品吗？")) {
                //拼接ID
                $.each(zhi, function () {
    ...
    function del(pid,page) {
        if (confirm("确定删除吗")) {
            ...
            $.ajax({
                ...
                data: {"pid": pid,"page": page,"proName":pname,"proType":typeid,"proLoPrice":lprice,"proHiPrice":hprice},
                type: "post",
                dataType: "text",
                success: function (msg) {
                    alert(msg);
    ...
    function one(pid, ispage) {
        ...
        var lprice = $("#lprice").val();
        var hprice = $("#hprice").val();
        var str = "?pid="+pid+"&page="+ispage+"&proName="+pname+"&proType="+typeid+"&proLoPrice="+lprice+"&proHiPrice=" + hprice;
...
<script type="text/javascript">
    function ajaxsplit(page) {
        /**
         * 因为这边只传入了页码page，如果不取出所有的查询条件，那么只会实现 带页码查询
         * */
        //取出查询条件
        var pname = $("#pname").val();
        var typeid = $("#typeid").val();
        ...
        $.ajax({
            ...
            data: {"page": page,"proName":pname,"proType":typeid,"proLoPrice":lprice,"proHiPrice":hprice},
            type: "post",
            success: function () {
                //重新加载分页显示的组件table
                $("#table").load("http://localhost:8080/mimissm/admin/product.jsp #table");
...
<script type="text/javascript">
    function condition(){
     ...
     $.ajax({
         type:"post",
         url:"${pageContext.request.contextPath}/prod/ajaxsplit.action",
         data:{"proName":pname,"proType":typeid,"proLoPrice":lprice,"proHiPrice":hprice},
         success:function (){
             //数据刷新
             $("#table").load("http://localhost:8080/mimissm/admin/product.jsp #table");
         }
